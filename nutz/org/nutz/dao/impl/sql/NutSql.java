...
public class NutSql extends NutStatement implements Sql {
	...
	public String toString() {
		 ...
		 * 
		 * // 填充参数 VarIndex vIndex = literal.getParamIndexes(); VarSet vs = rows.get(0); for (String name : vIndex.names()) { int[] is = vIndex.indexesOf(name); String s = Sqls.formatFieldValue(vs.get(name)).toString(); for (int i : is) ss[i] = s; }
		 * 
		 * return Lang.concat(ss).toString();
		 */
