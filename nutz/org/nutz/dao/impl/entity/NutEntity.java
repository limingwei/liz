...
public class NutEntity<T> implements Entity<T> {
		...
		if (!Lang.isEmptyArray(names) && names.length > 1) {
			for (String name : names) {
				if (byJava.containsKey(name) && byJava.get(name).isCompositePk())
					...
					throw Lang.makeThrow("Fail to find comosite field '%s' in class '%s'!", name, type.getName());
			}
	...
	public List<LinkField> getLinkFields(String regex) {
		...
		List<LinkField> re = new ArrayList<LinkField>(reOnes.size() + reManys.size() + reManymanys.size());
		re.addAll(reOnes);
		re.addAll(reManys);
