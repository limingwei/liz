...
public class OneLinkField extends AbstractLinkField implements LinkField {
	...
	public OneLinkField(Entity<?> entity, EntityHolder holder, LinkInfo info) {
		...
		// 链接实体的字段 - 应该是主键
		linkedField = hostField.getTypeMirror().isIntLike() ? this.getLinkedEntity().getIdField() : this.getLinkedEntity().getNameField();
		if (null == linkedField)
			throw Lang.makeThrow("Fail to find linkedField for @One(field=%s) '%s' : %s<=>%s", info.one.field(), this.getName(), this.getEntity().getType(), this.getLinkedEntity().getType());
