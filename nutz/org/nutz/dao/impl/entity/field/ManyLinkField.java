...
public class ManyLinkField extends AbstractLinkField implements LinkField {
	...
	public ManyLinkField(Entity<?> entity, EntityHolder holder, LinkInfo info) {
		...
		// 根据一个字段的值链接
			...
			// 宿主实体的字段 - 应该是主键
			hostField = linkedField.getTypeMirror().isIntLike() ? this.getEntity().getIdField() : this.getEntity().getNameField();
			if (null == hostField)
	...
	public Condition createCondition(Object host) {
		return null == linkedField ? null : Cnd.where(linkedField.getName(), "=", hostField.getValue(host));
	}
