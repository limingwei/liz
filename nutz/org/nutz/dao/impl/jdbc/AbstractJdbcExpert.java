...
public abstract class AbstractJdbcExpert implements JdbcExpert {
	...
	public boolean dropEntity(Dao dao, Entity<?> en) {
		...
		try {
			...
			if (tableName.equals(viewName)) {
				...
				dao.execute(Sqls.create("DROP VIEW " + viewName), Sqls.create("DROP TABLE " + tableName));
			}
		} catch (Exception e) {
			return false;
		}
	...
	protected String evalFieldType(MappingField mf) {
		...
		throw Lang.makeThrow("Unsupport colType '%s' of field '%s' in '%s' ", mf.getColumnType(), mf.getName(), mf.getEntity().getType().getName());
	}
