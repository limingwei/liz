...
public interface Dao {
	...
	/**
	 * 这个方法试图给你最大的灵活性，因为你的 ConnCallback 实现类将得到一个 Connection 接口 的实例。请注意，你不需要关闭这个连接，这个函数在退出时会替你关闭连接。
	 * <p>
	 ...
	 * 查询出一组记录。
	 * 
	 * @param tableName 表名 - 格式为 <b>tableName[:idName]</b> 比如 ： <b>t_pet</b> 或者 <b>t_pet:id</b> 尤其在 SqlServer2005 的环境下，需要用 t_pet:id 的形式来指明 ID 字段，否则 不能分页
	 * @param cnd 条件 - <b style=color:red>请注意：</b> 你传入的 Criteria 实现必须考虑到 没有 'Entity<?>' 传入。即 toSql 函数的参数永远为 null。
	 * @param pager 翻页信息
	 ...
	 * 
	 * @param classOfT 对象类型
	 * @param funcName 计算函数名，请确保你的数据是支持这个函数的
