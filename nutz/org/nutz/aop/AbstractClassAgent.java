...
public abstract class AbstractClassAgent implements ClassAgent {
	...
	protected abstract <T> Class<T> generate(ClassDefiner cd, Pair2[] pair2s, String newName, Class<T> klass, Constructor<T>[] constructors);
	...
	private <T> Pair2[] findMatchedMethod(Class<T> klass) {
		...
		for (Method m : all) {
			...
			if (mod == 0 || Modifier.isStatic(mod) || Modifier.isPrivate(mod) || Modifier.isFinal(mod) || Modifier.isAbstract(mod))
				continue;
